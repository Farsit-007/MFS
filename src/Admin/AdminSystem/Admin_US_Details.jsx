...
const Admin_US_Details = () => {
    const { phone } = useParams()
    const axiosSecure = useAxiosSecure()
    const { data: details, isLoading } = useQuery({
        queryKey: ['details', phone],
        queryFn: async () => {
            const { data } = await axiosSecure.get(`/US-details/${phone}`)
            return data
        }
    })
    const { data: cash = {} } = useQuery({
        queryKey: ['cash', phone],
        queryFn: async () => {
            const { data } = await axiosSecure.get(`/money-count/${phone}`)
            return data
        }
    })
    return (
        ...
        <div className="flex justify-between items-center px-5">
            ...
            <div>
            <h1 className="text-2xl p-2 px-5 text-white font-bold ">Total Send Money : {cash.send} Tk.</h1>
...
export default Admin_US_Details;