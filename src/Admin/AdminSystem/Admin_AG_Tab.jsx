...
import Admin_AG_Table from "./Admin_AG_Table";
...
const Admin_AG_Tab = () => {
    ...
    const { data: allusers = [], refetch, isLoading } = useQuery({
        queryKey: ['allusers'],
        queryFn: async () => {
            const { data } = await axiosSecure.get(`/AD-AG-Num`);
            return data;
        }
    });
    return (
                            ...
                            <tbody>
                                {
                                    allusers.slice(0,20).map((user) =>
                                        <Admin_AG_Table key={user._id} user={user} refetch={refetch}></Admin_AG_Table>
                                    )
                                }
                            </tbody>
                        </table>
